  ...
  return (
    <div className="bg-surface p-4 rounded-lg">
      <div className="flex items-center mb-3">
        <div className="w-10 h-10 rounded-full bg-primary-cyan/20 text-primary-cyan flex items-center justify-center font-bold text-sm mr-3 flex-shrink-0">
          {post.author.name.substring(0, 2).toUpperCase()}
        </div>
        <div>
          <Link to={`/profile/${post.author.address}`} className="font-bold hover:underline text-text text-sm sm:text-base">{post.author.name}</Link>
